...
void ExampleDriver::HMDDevice::Update()
{
    ...
    // Get orientation
    this->rot_y_ += (1.0f * (false) - 1.0f * (false)) * delta_seconds;
    this->rot_x_ += (-1.0f * (false) + 1.0f * (false)) * delta_seconds;
    ...
    linalg::vec<float, 3> forward_vec{-1.0f * (false) + 1.0f * (false), 0, 0};
    linalg::vec<float, 3> right_vec{0, 0, 1.0f * (false) - 1.0f * (false) };
